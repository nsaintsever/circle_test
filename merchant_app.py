...
def get_all_champ_codes_and_names_merchant(): # Renamed for clarity
    ...
    try:
        ...
        for f in os.listdir(effective_csv_dir):
            if f.startswith("C") and f.endswith(".csv"):
                ...
                if not code_match: # Try simple C<number>
    ...
    if not champs:
        st.warning("No champ CSV files found or parsed in Merchant App.")
        return {}

    sorted_champs = dict(sorted(champs.items(), key=sort_key_for_champ_code))
    return sorted_champs

ALL_CHAMPS_INFO = get_all_champ_codes_and_names_merchant()
